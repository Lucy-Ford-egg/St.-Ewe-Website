...
export const NavigationLink = ({text, navigationItemUrl, buttonType = "contained", color = "primary"}) => {
  ...
  if(navigationItemUrl?.internalnavigationItemUrl !== null){
    ...
    linkType = <Button sx={{textAlign: 'center' }} variant={buttonType} color={color} href={navigationItemUrl.externalnavigationItemUrl?.href} aria-label={`Link to ${text}`}>{text}</Button>
  }
  return(linkType)
}
