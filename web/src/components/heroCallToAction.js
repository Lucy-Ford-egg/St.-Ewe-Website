...
export const HeroCallToAction = ({ title, text, image, linkGroup }) => {
  ...
  return (
    <Container maxWidth="false" sx={{ px: {xs: 0}, py: { xs: 2, md: 6 }, display: 'grid', gridTemplateColumns: '5% auto 5%', gridTemplateRows: {xs: '30px 1fr 1fr 1fr 30px', md: 'repeat(6, 1fr)'} }}>
      <Container maxWidth="sm" sx={{ backgroundColor: 'primary.main', gridColumn: {xs: '2/2', md: '1/3'}, gridRow: {xs: '2/5', md: '2/6'}, position: 'relative', zIndex: 1, pt: {xs: 6, md: 6}, pb: {xs: 6, md: 3}, px: {xs: 0, md: 10}, mb: {xs: 0, md: -8} }}>
      ...
      <Container maxWidth="xl" disableGutters={isMobile ? true : false} sx={{ px: {xs: 0}, gridColumn: '1/4', gridRow: '1/6' }}>
        <GatsbyImage layout="contained" aspectRation="133/8" style={{ minHeight: '100%' }} image={getImage(image.asset)} alt="alt tag" />
      </Container>
    </Container>
  )
